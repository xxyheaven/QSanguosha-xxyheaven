...
public:
    enum GuanxingType
    {
        GuanxingUpOnly = 1, GuanxingBothSides = 0, GuanxingDownOnly = -1, GuanxingNone = -2
    };
    ...
    void revivePlayer(ServerPlayer *player, bool sendlog = false, bool emotion = true);
    ...
    ServerPlayer *getLord(bool as_shown = false) const;
    ...
    AskForMoveCardsStruct askForMoveCards(ServerPlayer *zhuge, const QList<int> &upcards, const QList<int> &downcards, bool visible, const QString &reason,
        const QString &pattern, int min_num, int max_num, bool can_refuse = true, bool moverestricted = false,const QList<int> &notify_visible_list = QList<int>());
    ...
    void sendCompulsoryTriggerLog(ServerPlayer *player, const QString &skill_name, bool is_hero_skill = true);
    ...
    void acquireSkill(ServerPlayer *player, const Skill *skill, bool open = true, bool turn_clear = false);
    void acquireSkill(ServerPlayer *player, const QString &skill_name, bool open = true, bool turn_clear = false);
    void adjustSeats();
    void swapPile();
    ...
    ServerPlayer *findPlayer(const QString &object_name, bool include_dead = false) const;
	ServerPlayer *findPlayerByGeneralName(const QString &general_name, bool include_dead = false) const;
    ...
    bool askForNullification(const CardEffectStruct &effect);
	...
	QList<int> askForCardsChosen(ServerPlayer *player, ServerPlayer *who, const QString &flags, const QString &reason, int min_num, int max_num,
        bool handcard_visible = false, Card::HandlingMethod method = Card::MethodNone, const QList<int> &disabled_ids = QList<int>());
    const Card *askForCard(ServerPlayer *player, const QString &pattern, const QString &prompt, const QVariant &data, const QString &skill_name, ServerPlayer *to = NULL);
    ...
    const Card *askForUseCard(ServerPlayer *player, const QString &pattern, const QString &prompt, const QVariant &data = QVariant(),
    ...
    const Card *askForUseSlashTo(ServerPlayer *slasher, ServerPlayer *victim, const QString &prompt,
        bool distance_limit = true, bool disable_extra = false, bool addHistory = false, const QVariant &data = QVariant());
    ...
    void setPlayerSkinId(ServerPlayer *player, int skin_id, bool is_head, bool just_set = false);
...
private:
	...
	QStack<CardEffectStruct> m_trickStack;
    CardEffectStruct m_trickEffect;
    SlashEffectStruct m_slashEffect;
    bool game_started;
    bool game_finished;
    bool game_paused;
    lua_State *L;
