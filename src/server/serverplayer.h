...
public:
    ...
    void broadcastSkillInvoke(const QString &card_name, int type = -1) const;
    int getRandomHandCardId() const;
    const Card *getRandomHandCard() const;
    void obtainCard(const Card *card, bool unhide = true);

    void sealAreas(const QString &area_name);
    void throwAllEquips();
    ...
    void drawCards(int n, const QString &reason = QString(), bool from_up = true);
    ...
    int getCardUsedTimes(const QString &pattern) const;
    int getCardRespondedTimes(const QString &pattern) const;
