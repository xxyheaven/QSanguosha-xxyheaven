...
int Player::getAttackRange(bool include_weapon) const
{
    ...
    if (include_weapon) {
        ...
        Q_ASSERT(card);
        original_range = card->getRange();
    }

    int real_range = original_range + Sanguosha->correctAttackRange(this, include_weapon, false);
...
{
    return getRole() == "lord" && hasShownRole();
}
...
bool Player::hasSkill(const QString &skill_name, bool include_lose) const
{
    ...
    if (skills.contains(skill_name) || acquired_skills.contains(skill_name)) return true;
...
bool Player::canSetEquip(int index) const
{
    ...
    return getEquip(index) == NULL && getMark(area_names.at(index)) == 0;
}
...
{
    if (this != to && hasSkill("benxi") && getPhase() != Player::NotActive) {
        ...
        if (alladj) return true;

    }

    return property("IngoreArmor").toStringList().contains(to->objectName());
}
...
bool Player::canGetCard(const Player *to, int card_id) const
{
    Q_ASSERT(to);
    if (this == to) {
        foreach (const Card *c, to->getHandcards()) {
            if (c->getEffectiveId() == card_id) {
                return false;
            }
        }
    }
...
#define THIS_SLASH (slash == NULL ? newslash : slash)
    ...
    valid_slash_count += Sanguosha->correctCardTarget(TargetModSkill::Residue, this, THIS_SLASH, NULL);
...
bool Player::isCardLimited(const Card *card, Card::HandlingMethod method, bool isHandcard) const
{
    ...
    return Sanguosha->isCardLimited(this, card, method);
}
