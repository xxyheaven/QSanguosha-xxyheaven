...
Card::Card(Suit suit, int number, bool target_fixed)
    ...
    will_throw(true), has_preact(false), can_recast(false), will_sort(true),
...
bool Card::sameNumberWith(const Card *other) const
{
    return getNumber() > 0 && getNumber() == other->getNumber();
}
...
int Card::getNumber() const
{
    if (isVirtualCard()) {
        if (subcardsLength() == 1)
            return Sanguosha->getCard(getEffectiveId())->getNumber();
        else
            return 0;
    }
    return m_number;
...
bool Card::sameSuitWith(const Card *other) const
{
    Card::Suit suit = getSuit();
    if (suit != Spade && suit != Club && suit != Heart && suit != Diamond) return false;
...
{
    return getColor() != Colorless && getColor() == other->getColor();
}
...
QString Card::toString(bool hidden) const
{
    ...
    if (!isVirtualCard())
        ...
        .arg(getSuitString()).arg(getNumberString()).arg(subcardString()).arg(costcardString());
}
...
QString Card::costcardString() const
{
    if (costcards.isEmpty())
        return ".";

    QStringList str;
    foreach(int subcard, costcards)
        str << QString::number(subcard);
...
void Card::addCostcards(const QList<const Card *> &cards)
{
    foreach(const Card *card, cards)
        costcards.append(card->getId());
}

void Card::addCostcards(const QList<int> &subcards_list)
{
    costcards.append(subcards_list);
}
...
const Card *Card::Parse(const QString &str)
{
    ...
    if (str.startsWith(QChar('@'))) {
        ...
        QStringList subcard_ids, costcard_ids;
        if (subcard_str != ".")
            subcard_ids = subcard_str.split("+");
		if (texts.length() > 6) {
			QString costcard_str = texts.at(6);
			if (costcard_str != ".")
				costcard_ids = costcard_str.split("+");
		}
        ...
        card->addSubcards(StringList2IntList(subcard_ids));
		card->addCostcards(StringList2IntList(costcard_ids));
...
void Card::onUse(Room *room, const CardUseStruct &use) const
{
    ...
    if (card_use.to.isEmpty())
        card_use.to = this->defaultTargets(room, card_use.from);
    ...
    if (card_use.m_isOwnerUse && !card_use.card->isMute()) {
        ...
        if (card_use.card->isKindOf("Collateral")) {
            ...
            if (victim)
                log.to.prepend(victim);
        } else if (card_use.card->isKindOf("BeatAnother")) {
            ServerPlayer *victim = card_use.to.first()->tag["beatAnotherTo"].value<ServerPlayer *>();
            if (victim)
                log.to.prepend(victim);
        }
        log.type = "#UseCard";
...
void Card::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &targets) const
{
    ...
    QVariantList nullified2_list = tag["Nullified_List"].toList();
    ...
    foreach (ServerPlayer *target, targets) {
        if (all_nullified || nullified_list.contains(target->objectName()) || nullified2_list[index].toBool()) {
            QVariantList qinggang_list = tag["Qinggang_List"].toList();
...
void Card::addCostcard(int card_id)
{
    if (card_id < 0)
        qWarning("%s", qPrintable(tr("Cost must not be virtual card!")));
    else
        costcards << card_id;
}

void Card::addCostcard(const Card *card)
{
    addCostcard(card->getEffectiveId());
}

QList<int> Card::getCostcards() const
{
    return costcards;
}
...
bool Card::usecontains(int card_id) const
{
    return subcards.contains(card_id) || costcards.contains(card_id);
}

bool Card::usecontains(const Card *card) const
{
    usecontains(card->getEffectiveId());
}
...
const Card *Card::validateInResponse(ServerPlayer *user, QList<ServerPlayer *> &targets) const
{
	if (targets.isEmpty())
		return validateInResponse(user);
	return this;
}
...
void SkillCard::extraCost(Room *room, const CardUseStruct &card_use) const
{
    if (card_use.card->willThrow()) {
		...
		CardMoveReason reason1(CardMoveReason::S_REASON_REMOVE_FROM_PILE, QString(), card_use.from->objectName(), card_use.card->getSkillName(), QString());
        CardMoveReason reason2(CardMoveReason::S_REASON_THROW, card_use.from->objectName(), QString(), card_use.card->getSkillName(), QString());

		QList<CardsMoveStruct> throw_move;
		if (!cards1.isEmpty()) {
			CardsMoveStruct move(cards1, NULL, Player::DiscardPile, reason1);
			throw_move.append(move);
		}
		if (!cards2.isEmpty()) {
			CardsMoveStruct move(cards2, NULL, Player::DiscardPile, reason2);
			throw_move.append(move);
		}
		if (!throw_move.isEmpty())
			room->moveCardsAtomic(throw_move, true);
    }
}
