...
void CangjiCard::onEffect(const CardEffectStruct &effect) const
{
    ...
    if (loop)
        room->askForUseCard(effect.from, "@@cangji", "@cangji-install", QVariant(), Card::MethodNone);
}
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (room->getMode() == "02_1v1") {
            ...
            room->askForUseCard(player, "@@cangji", "@cangji-install", QVariant(), Card::MethodNone);
        }
        return false;
    }
};
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        foreach (ServerPlayer *p, use.to) {
            ...
            if (player->getHp() <= handcardnum && player->askForSkillInvoke(this, QVariant::fromValue(p))) {
                player->broadcastSkillInvoke("liegong");
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *sunshangxiang, QVariant &data) const
    {
        ...
        if (move.from->getPhase() != Player::NotActive && move.to_place == Player::DiscardPile) {
            ...
            if (card_ids.isEmpty())
                ...
                sunshangxiang->broadcastSkillInvoke("yinli");
...
public:
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if (triggerEvent == DrawInitialCards) {
            ...
            player->broadcastSkillInvoke("cuorui", 1);
            ...
            if (change.to == Player::Judge && player->getMark("CuoruiSkipJudge") == 0) {
                player->broadcastSkillInvoke("cuorui", 2);
                room->sendCompulsoryTriggerLog(player, "cuorui");
