...
void Chengxiang::onDamaged(ServerPlayer *target, const DamageStruct &damage) const
{
    ...
    AskForMoveCardsStruct result = room->askForMoveCards(target, card_ids, QList<int>(), true, objectName(), "sum" + QString::number(total_point), 1, 4, false, true);
	...
	if (!card_to_throw.isEmpty()) {
		...
		CardMoveReason reason(CardMoveReason::S_REASON_NATURAL_ENTER, target->objectName(), "chengxiang", QString());
...
public:
    ...
    virtual bool effect(TriggerEvent , Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *caochong) const
    {
        if (room->askForCard(caochong, ".Equip", "@renxin-card:" + player->objectName(), data, objectName(), player)) {
            caochong->turnOver();
            LogMessage log;
            log.type = "#Renxin";
            log.from = player;
...
public:
    ...
    {
        return TriggerSkill::triggerable(target) && target->getPhase() == Player::Play && target->getCardUsedTimes(".") >= target->getHp();
    }
        ...
        if (use.card->isNDTrick() || use.card->isKindOf("BasicCard")) {
            ...
            if (!use.card->isKindOf("AOE") && !use.card->isKindOf("GlobalEffect")) {
                ...
                foreach (ServerPlayer *p, room->getAlivePlayers()) {
                    if (!use.to.contains(p) && !room->isProhibited(jianyong, p, use.card) && use.card->targetFilter(QList<const Player *>(), p, jianyong))
                        available_targets << p;
                }
                room->setCardFlag(use.card, "-Global_NoDistanceChecking");
            }
            ...
            QString choice = room->askForChoice(jianyong, "qiaoshui", choices.join("+"), data, QString(), "add+remove+cancel");
            if (choice == "cancel")
                return false;
            else if (choice == "add") {
...
public:
    ...
    {
        if (card->isKindOf("XiansiCard")) {
            Room *room = player->getRoom();
            QString tag_name = QString("AudioEffect:liufeng-xiansi=1+2");
    ...
    bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if ((triggerEvent == GameStart && TriggerSkill::triggerable(player))
            ...
            foreach (ServerPlayer *liufeng, liufengs) {
                if (player != liufeng && !player->hasSkill("xiansi_slash")) {
...
const Card *XiansiSlashCard::validate(CardUseStruct &cardUse) const
{
    ...
    log.arg = "xiansi";
    room->sendLog(log);
    room->broadcastSkillInvoke("xiansi", liufeng, QList<int>() << 3 << 4);
    room->notifySkillInvoked(liufeng, "xiansi");
...
public:
    ...
    virtual void onDamaged(ServerPlayer *target, const DamageStruct &damage) const
    {
        ...
        QVariant data = QVariant::fromValue(damage);
        Room *room = target->getRoom();
        if (room->askForCard(target, "..", "@duodao-get", data, objectName(), damage.from)) {
...
void MiejiCard::extraCost(Room *room, const CardUseStruct &card_use) const
{
    CardMoveReason reason(CardMoveReason::S_REASON_PUT, card_use.from->objectName(), QString(), "mieji", QString());
    room->moveCardTo(this, card_use.from, NULL, Player::DrawPile, reason, true);
}
...
void FenchengCard::onUse(Room *room, const CardUseStruct &card_use) const
{
    ...
    : Package("YJCM2013")
{
	...
	liufeng->addSkill(new DetachEffectSkill("xiansi", "counter"));
    related_skills.insertMulti("xiansi", "#xiansi-attach");
	related_skills.insertMulti("xiansi", "#xiansi-clear");
