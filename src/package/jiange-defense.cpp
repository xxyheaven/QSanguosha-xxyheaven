...
public:
    ...
    virtual bool onPhaseChange(ServerPlayer *target) const
    {
        ...
        if (defensive_machine && defensive_machine->isWounded()){
            if (room->askForSkillInvoke(target, objectName(), "prompt1:" + defensive_machine->objectName())){
				room->broadcastSkillInvoke(objectName());
				room->doAnimate(QSanProtocol::S_ANIMATE_INDICATE, target->objectName(), defensive_machine->objectName());
				room->recover(defensive_machine, RecoverStruct(target));
			}
        }else if (offensive_machine){
            if (room->askForSkillInvoke(target, objectName(), "prompt2:" + offensive_machine->objectName())){
				room->broadcastSkillInvoke(objectName());
				room->doAnimate(QSanProtocol::S_ANIMATE_INDICATE, target->objectName(), offensive_machine->objectName());
...
public:
    ...
    {
        return from && isJianGeFriend(from, to) && card->isKindOf("ArcheryAttack") && card->getSkillName() == "jgmojian";
    }
};
    ...
    : Package("JianGeDefense")
{
    ...
    Machine *jg_machine_fudibian = new Machine(this, "jg_machine_fudibian", "wei", 3, true, true);
    jg_machine_fudibian->addSkill("jgjiguan");
    jg_machine_fudibian->addSkill(new JGDidong);

    Machine *jg_machine_lieshiyazi = new Machine(this, "jg_machine_lieshiyazi", "wei", 5, true, true);
    jg_machine_lieshiyazi->addSkill("jgjiguan");
    jg_machine_lieshiyazi->addSkill(new JGNailuo);
    ...
    Soul *jg_soul_huangyueying = new Soul(this, "jg_soul_huangyueying", "shu", 3, false, true);
    ...
    jg_machine_jileibaihu->addSkill("vszhenwei");
    jg_machine_jileibaihu->addSkill(new JGBenlei);

    Machine *jg_machine_lingjiaxuanwu = new Machine(this, "jg_machine_lingjiaxuanwu", "shu", 4, true, true);
