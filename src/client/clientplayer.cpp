...
void ClientPlayer::setMark(const QString &mark, int value, bool is_tip)
{
    ...
    } else if (mark.endsWith("Sealed")) {
        if (mark == "JudgeSealed") {
            if (value > 0)
                emit judgearea_sealed();
            else
                emit judgearea_unsealed();
        } else {
            QString area_name = mark;
            area_name.chop(6);
            if (value > 0)
                emit equiparea_sealed(area_name);
            else
...
{
    if (id == headSkinId || id > general->skinCount())
        return;
...
{
    if (deputySkinId == id || id > general2->skinCount())
        return;
