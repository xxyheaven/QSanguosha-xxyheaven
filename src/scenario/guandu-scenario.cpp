...
{
    effect.from->broadcastSkillInvoke("tuxi");
...
public:
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        case GameStart: {
            ...
            ServerPlayer *caocao = room->findPlayerByGeneralName("caocao");
            room->installEquip(caocao, "qinggang_sword");
            room->installEquip(caocao, "zhuahuangfeidian");

            ServerPlayer *liubei = room->findPlayerByGeneralName("liubei");
            room->installEquip(liubei, "double_sword");

            ServerPlayer *guanyu = room->findPlayerByGeneralName("guanyu");
            room->installEquip(guanyu, "blade");
            room->installEquip(guanyu, "chitu");
            room->acquireSkill(guanyu, "zhanshuangxiong");


            ServerPlayer *zhangliao = room->findPlayerByGeneralName("nos_zhangliao");
        ...
        case Damaged: {
            ...
            if (player->getGeneralName() == "yuanshao" && damage.nature == DamageStruct::Fire
                ...
                foreach (QString name, tos) {
                    ServerPlayer *to = room->findPlayerByGeneralName(name);
...
void GuanduScenario::onTagSet(Room *room, const QString &) const
{
    ...
    if (zhanshuangxiong && burnwuchao) {
        ServerPlayer *guojia = room->findPlayerByGeneralName("nos_guojia");
