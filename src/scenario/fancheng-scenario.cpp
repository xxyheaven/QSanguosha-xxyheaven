...
public:
    ...
    virtual bool onPhaseChange(ServerPlayer *target) const
    {
        if (target->getPhase() == Player::Start) {
            ...
            room->askForUseCard(target, "@@dujiang", "@dujiang-card", QVariant(), Card::MethodDiscard);
        }
...
public:
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        case GameStart: {
            ...
            ServerPlayer *sp_pangde = room->findPlayerByGeneralName("sp_pangde");
            room->acquireSkill(sp_pangde, "taichen_fight");

            ServerPlayer *nos_huatuo = room->findPlayerByGeneralName("nos_huatuo");
            room->installEquip(nos_huatuo, "hualiu");
            room->acquireSkill(nos_huatuo, "guagu");

            ServerPlayer *lvmeng = room->findPlayerByGeneralName("lvmeng");
            room->acquireSkill(lvmeng, "dujiang");

            ServerPlayer *caoren = room->findPlayerByGeneralName("caoren");
...
{
    if (key == "Flood") {
        ServerPlayer *xuhuang = room->findPlayerByGeneralName("xuhuang");
        if (xuhuang) {
