...
GeneralCardItem::GeneralCardItem(const QString &generalName, const int skinId)
    : CardItem(generalName), hasCompanion(false), expand_type(None)
{
    _skinId = skinId;
...
void GeneralCardItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
    ...
    if (hasCompanion) {
		QPixmap icon = G_ROOM_SKIN.getPixmap(QSanRoomSkin::S_SKIN_KEY_GENERAL_CARD_ITEM_COMPANION_ICON);
		painter->drawPixmap(boundingRect().center().x() - icon.width() / 2 + 2, boundingRect().bottom() - icon.height(), icon);
	}

    if (expand_type == ExpandUp || expand_type == ExpandDown) {
		QPixmap arrows = G_ROOM_SKIN.getPixmap(QSanRoomSkin::S_SKIN_KEY_GENERAL_CARD_ITEM_ARROWS, (expand_type == ExpandUp) ? "up" : "down");
        int y = (expand_type == ExpandUp) ? (boundingRect().top() - arrows.height() / 3) : (boundingRect().bottom() - arrows.height() * 2 / 3);
        painter->drawPixmap(boundingRect().center().x() - arrows.width() / 2 + 2, y, arrows);
	}
}
...
void GeneralCardItem::setExpandType(ExpandType type)
{
    if (expand_type == type) return;
    expand_type = type;
    update();
}
...
QRectF ChooseGeneralBox::boundingRect() const
{
    ...
    int width = first_row * G_COMMON_LAYOUT.m_cardNormalWidth + (first_row - 1) * card_to_center_line + left_blank_width * 2;

    if (lord_item != NULL)
	    width += G_COMMON_LAYOUT.m_cardNormalWidth + lord_to_card_center_line;
...
*/
void ChooseGeneralBox::chooseGeneral(const QStringList &_generals, bool view_only, bool single_result, const QString &reason, bool convert_enabled)
{
    //repaint background
    QStringList all_generals = _generals;
    ...
    if (convert_enabled) {
        foreach (QString name, all_generals) {
            QString main_general = Sanguosha->getMainGeneral(name);
            if (generals.contains(main_general)) continue;
            bool has_append = false;
            foreach (QString _name, Sanguosha->getConvertGenerals(main_general)) {
                if (generals.contains(_name))
                    has_append = true;
            }
            if (has_append) continue;
            generals << name;
        }
    } else
    ...
    foreach (const QString &general, generals) {
		...
		connect(general_item, &GeneralCardItem::enter_hover, this, &ChooseGeneralBox::showConvertion);
...
/*
    ...
    int x_fixed = lord_name.isEmpty() ? 0 : (card_width + lord_to_card_center_line);
	...
	if (!lord_name.isEmpty()) {
        ...
        this->lord_item = new GeneralCardItem(lord_name, skinId);
        lord_item->setFlag(QGraphicsItem::ItemIsFocusable);
        lord_item->setZValue(z--);

        if (view_only || single_result) {
            lord_item->setFlag(QGraphicsItem::ItemIsMovable, false);
        } else {
            lord_item->setAutoBack(true);
            connect(lord_item, &GeneralCardItem::released, this, &ChooseGeneralBox::_adjust);
        }
/*
        if (!view_only) {
            connect(lord_item, &GeneralCardItem::clicked, this, &ChooseGeneralBox::_onItemClicked);
            if (!single_result)
                connect(lord_item, &GeneralCardItem::general_changed, this, &ChooseGeneralBox::adjustItems);
        }
		...
		lord_item->showCompanion();

        lord_item->setParentItem(this);

		QPointF pos;

        pos.setX(left_blank_width + card_width / 2);
        pos.setY(top_blank_width + card_height / 2 + ((items.length() > first_row) ? ((card_height + card_to_center_line) / 2) : 0));

        lord_item->setPos(pos);
    ...
    for (int i = 0; i < items.length(); ++i) {
        ...
        QPointF pos;
        if (i < first_row) {
            pos.setX(left_blank_width + (card_width + card_to_center_line) * i + card_width / 2 + x_fixed);
            pos.setY(top_blank_width + card_height / 2);
        } else {
            if (items.length() % 2 == 1) {
                pos.setX(left_blank_width + card_width / 2 + card_to_center_line / 2
                    + (card_width + card_to_center_line) * (i - first_row) + card_width / 2 + x_fixed);
            } else {
                pos.setX(left_blank_width + (card_width + card_to_center_line) * (i - first_row) + card_width / 2 + x_fixed);
            }
...
void ChooseGeneralBox::showConvertion()
{
    CardItem *card_item = qobject_cast<CardItem *>(sender());
    if (card_item) {
        QString	general_name = card_item->objectName();
		if (_m_current_convert_name != general_name) {
			changeConvertion(_m_current_convert_name, false);
			changeConvertion(general_name, true);
			_m_current_convert_name = general_name;
		}
	}
}
...
void ChooseGeneralBox::createConvertion(GeneralCardItem *card_item, bool first_row, QStringList all_generals)
{
    ...
    int i = 0;
    foreach (QString convert_name, convert_names) {
        if (!all_generals.contains(convert_name)) continue;
		GeneralCardItem *general_item = new GeneralCardItem(convert_name, 0);
		general_item->setFlag(QGraphicsItem::ItemIsFocusable);
        general_item->setZValue(card_item->zValue());
		general_item->setFlag(QGraphicsItem::ItemIsMovable, false);
		general_item->setParentItem(this);
        QPointF pos;
        pos.setX(card_item->homePos().x() + (G_COMMON_LAYOUT.m_cardNormalWidth + card_to_center_line) * i);
        pos.setY(card_item->homePos().y() + (first_row ? (-1) : 1)*(G_COMMON_LAYOUT.m_cardNormalHeight + lord_to_card_center_line));
        general_item->setPos(pos);
...
void ChooseGeneralBox::reply()
{
	...
	QString	generals = item->objectName();
