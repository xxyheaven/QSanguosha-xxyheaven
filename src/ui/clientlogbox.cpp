    ...
    QString card_str, QString arg, QString arg2)
{
    ...
    if (type.startsWith("#UseCard") && !card_str.isEmpty() && !from_general.isEmpty()) {
        ...
        if (card == NULL) return;
        if (card->isKindOf("SingleTargetTrick") && tos.length() > 1) {
            to = ClientInstance->getPlayerName(tos.last());
            to = bold(to, Qt::red);
        }

        // do Indicator animation
        if ((card->isKindOf("LijianCard") || card->isKindOf("SingleTargetTrick")) && tos.length() == 2) {
        ...
        if (card->isVirtualCard()) {
            ...
            if (card->getTypeId() == Card::TypeSkill && !card->isKindOf("YanxiaoCard")) {
                ...
                if (card->getSkillName().contains("guhuo"))
                    return;
