    ...
    : main_window(main_window), game_started(false), m_tableBgPixmap(1, 1), m_tableBgPixmapOrig(1, 1)
{
    ...
    //connect(Self, SIGNAL(count_changed(QString, int)), dashboard, SLOT(updateCount(QString, int)));
    //connect(Self, SIGNAL(tip_changed(QString, bool)), dashboard, SLOT(updateTip(QString, bool)));


    connect(Self, SIGNAL(equiparea_sealed(QString)), dashboard, SLOT(sealEquipArea(QString)));
    connect(Self, SIGNAL(equiparea_unsealed(QString)), dashboard, SLOT(unsealEquipArea(QString)));
    ...
    connect(ClientInstance, SIGNAL(generals_got(QStringList, bool, QString, bool)), this, SLOT(chooseGeneral(QStringList, bool, QString, bool)));
    ...
    connect(ClientInstance, SIGNAL(cards_got(const ClientPlayer *, QString, QString, bool, Card::HandlingMethod, QList<int>, QList<int>, int, int)),
        this, SLOT(chooseCard(const ClientPlayer *, QString, QString, bool, Card::HandlingMethod, QList<int>, QList<int>, int, int)));
    ...
    connect(ClientInstance, SIGNAL(pile_shown(QString, QList<int>, QString)), this, SLOT(moveCardToPile(QString, QList<int>, QString)));
    connect(ClientInstance, SIGNAL(dummy_shown(CardMoveReason)), this, SLOT(showDummy(CardMoveReason)));
    ...
    connect(m_cardContainer, SIGNAL(item_chosen(int)), ClientInstance, SLOT(onPlayerChooseAG(int)));
    connect(m_cardContainer, SIGNAL(item_gongxined(int)), ClientInstance, SLOT(onPlayerReplyGongxin(int)));
    ...
    connect(ClientInstance, SIGNAL(ag_cleared()), m_cardContainer, SLOT(clear()));
...
void RoomScene::handleGameEvent(const QVariant &args)
{
    ...
    case S_GAME_EVENT_PLAY_EFFECT: {
        ...
        Sanguosha->playAudioEffect(G_ROOM_SKIN.getPlayerAudioEffectPath(skillName, category, type, player), false);
        break;
    }
...
void RoomScene::enableTargetsAfterGuhuo()
{
    const Card *pendingcard = dashboard->pendingCard();
    const SkillCard *guhuo_card = qobject_cast<const SkillCard *>(pendingcard);
    SkillCard *card = const_cast<SkillCard *>(guhuo_card);
    QString guhuo_name = Self->tag[card->getSkillName()].toString();
    card->setUserString(guhuo_name);
    enableTargets(card);
}
...
void RoomScene::enableTargets(const Card *card)
{
    ...
    if (!enabled) {
        if (current_select_box)
            current_select_box->buttonFilter(card);
        ok_button->setEnabled(false);
        if (status == Client::Playing && !getDasboard()->currentSkill())
            cancel_button->setEnabled(false);
        return;
    }
    ...
    if (card == NULL) {
        ...
        if (current_select_box)
            current_select_box->buttonFilter();
        ok_button->setEnabled(false);
        if (status == Client::Playing && !getDasboard()->currentSkill())
            cancel_button->setEnabled(false);
        return;
    ...
    if (card->targetFixed()
        ...
        && (card->getTypeId() != Card::TypeSkill && status != Client::RespondingUse))
    ...
    if (current_select_box)
        current_select_box->buttonFilter(card, selected_targets);
    else
...
void RoomScene::chooseGeneral(const QStringList &generals, bool single_result, const QString &reason, bool convert_enabled)
{
    ...
    bool handcard_visible, Card::HandlingMethod method, QList<int> disabled_ids, QList<int> handcards, int min_num, int max_num)
{
    ...
    m_playerCardBox->chooseCard(reason, player, flags,
...
void RoomScene::getCards(int moveId, QList<CardsMoveStruct> card_moves)
{
    ...
    for (int i = 0; i < card_moves.size(); i++) {
        ...
        m_cardContainer->m_currentPlayer = (ClientPlayer *)movement.to;
...
void RoomScene::keepGetCardLog(const CardsMoveStruct &move)
{
    ...
    if (move.to_place == Player::PlaceSpecial && !move.to_pile_name.isNull() && !move.to_pile_name.startsWith('#')
            && move.from_place != Player::PlaceSpecial && move.reason.m_reason != CardMoveReason::S_REASON_SWAP) {
    ...
    if ((move.from_place == Player::PlaceTable || move.from_place == Player::PlaceJudge)
        && move.to_place == Player::PlaceHand && move.reason.m_reason != CardMoveReason::S_REASON_SWAP
    ...
    if (move.from && move.from_place != Player::PlaceHand && move.from_place != Player::PlaceJudge && move.from_place != Player::PlaceSpecial
        && move.to_place != Player::PlaceDelayedTrick && move.to_place != Player::PlaceJudge && move.to_place != Player::PlaceSpecial
...
void RoomScene::addSkillButton(const Skill *skill)
{
    ...
    if (select_options != "" && !m_replayControl) {
        if (select_options.startsWith("guhuo_")) {
            QString guhuo_type = select_options.mid(6);
            ...
            connect(guhuo, &GuhuoBox::onButtonClick, this, &RoomScene::enableTargetsAfterGuhuo);
        }
    }
...
void RoomScene::updateStatus(Client::Status oldStatus, Client::Status newStatus)
{
    foreach (QSanSkillButton *button, m_skillButtons) {
        ...
        if (vsSkill != NULL) {
            ...
            else if (newStatus == Client::AskForShowOrPindian)
                reason = CardUseStruct::CARD_USE_REASON_RESPONSE;
    ...
    case Client::Exchanging: {
        ...
        if (newStatus != Client::Exchanging)
            discard_skill->setIsGamerule(ClientInstance->m_isGameruleDiscard);
    ...
    int max = 0;
    if (Self->hasFlag("kuangcai") && newStatus == Client::Playing) {
        if (Self->getMark("#kuangcai") == 0) {
            doTimeout();
            return;
        }
        max = Self->getMark("#kuangcai");
    } else if (ServerInfo.OperationTimeout == 0)
        return;
    ...
    if ((newStatus & Client::ClientStatusFeatureMask) != Client::StatusHasOwnProgressBar
        ...
        dashboard->showProgressBar(ClientInstance->getCountdown(), max);
    }
}
...
void RoomScene::onSkillActivated()
{
    ...
    const ViewAsSkill *vs_skill = button->getViewAsSkill();

    if (vs_skill && !vs_skill->inherits("FilterSkill")) {
        cancel_button->setEnabled(true);
        Self->tag.remove(vs_skill->objectName());
        dashboard->startPending(vs_skill);
        const Skill *skill = button->getSkill();
        if (skill) {
            QString select_options = skill->getSelectBox();
            if (select_options != "" && !m_replayControl && !select_options.startsWith("guhuo_")) {
                SelectBox *select = new SelectBox(skill->objectName(), select_options.split("+"));
                select->setParent(this);
                addItem(select);
                select->setZValue(30001);
                return;
            }
        }
        const Card *card = dashboard->pendingCard();
...
void RoomScene::doCancelButton()
{
    ...
    case Client::Playing: {
        ...
        dashboard->unselectAll(NULL, false);
        if (skill)
            cancelViewAsSkill();
        else
            dashboard->stopPending();
...
{
    QApplication::alert(main_window);
    m_chooseGeneralBox->chooseGeneral(names, true, false, Sanguosha->translate(reason));
}
...
void RoomScene::killPlayer(const QString &who)
{
    ...
    if (Config.EnableEffects && Config.EnableLastWord && !Self->hasFlag("marshalling"))
        general->lastWord(ClientInstance->getPlayer(who)->getHeadSkinId());
    m_roomMutex.unlock();
}
...
void RoomScene::showDummy(const CardMoveReason &reason)
{
    QList<CardItem *> card_items;
    const Card *view_as_card = reason.m_extraData.value<const Card *>();
    if (view_as_card) {
        CardItem *item = new CardItem(view_as_card);
        item->setOpacity(0.0);
        card_items.append(item);
    }
    bringToFront(m_tablePile);
...
void RoomScene::showPile(const QList<int> &card_ids, const QString &name, const ClientPlayer *target)
{
    ...
    if (name == "huashen" && target->hasSkill("huashen")) {
...
void RoomScene::showIndicator(const QString &from, const QString &to, int secs)
{
    ...
    addItem(indicator);
    if (secs > 0)
        QTimer::singleShot(secs, indicator, SLOT(doAnimation()));
    else
