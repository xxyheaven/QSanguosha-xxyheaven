...
bool GeneralOverview::hasSkin(const QString &general_name)
{
    ...
    if (skin_index == 0)
        return G_ROOM_SKIN.generalHasSkin(general_name, 1, true);
    return true;
}
...
QString GeneralOverview::getCvInfo(const QString &general_name)
{
    int skinId = Config.value(QString("HeroSkin/%1").arg(general_name), 0).toInt();
    QString prefix = (skinId > 0) ? QString::number(skinId) : QString();
    QString cvText = Sanguosha->translate(QString("cv:%1%2").arg(prefix).arg(general_name));
    if (!cvText.startsWith("cv:")) {
        return cvText;
    } else {
        cvText = Sanguosha->translate("cv:" + general_name);
        if (!cvText.startsWith("cv:"))
            return cvText;
        else
            return tr("Official");
...
{
    const int skinId = Config.value(QString("HeroSkin/%1").arg(general_name), 0).toInt();
    QString prefix = (skinId > 0) ? QString::number(skinId) : QString();
    QString illustratorText = Sanguosha->translate(QString("illustrator:%1%2").arg(prefix).arg(general_name));
    if (!illustratorText.startsWith("illustrator:")) {
        return illustratorText;
    } else {
        illustratorText = Sanguosha->translate("illustrator:" + general_name);
        if (!illustratorText.startsWith("illustrator:"))
            return illustratorText;
        else
...
void GeneralOverview::addLines(const Skill *skill, const QString &general_name)
{
    QString skill_name = skill->objectName();
    int skinId = Config.value(QString("HeroSkin/%1").arg(general_name), 0).toInt();
	QString skill_text = Sanguosha->translate(skill_name);
    QStringList sources = Sanguosha->findSkillAudioFileNames(skill_name, general_name, skinId);

    if (sources.isEmpty()) {
        QCommandLinkButton *button = new QCommandLinkButton(skill_text);
        ...
        for (int i = 0; i < sources.length(); i++) {
            ...
            QString button_text = skill_text;
            if (sources.length() != 1)
...
{
    QString last_word;
    int skinId = Config.value(QString("HeroSkin/%1").arg(general->objectName()), 0).toInt();
...
void GeneralOverview::on_tableWidget_itemSelectionChanged()
{
    ...
    QPixmap pixmap;
    if (G_ROOM_SKIN.generalHasSkin(general_name, n, true)) {
        pixmap = G_ROOM_SKIN.getGeneralCardPixmap(general_name, n);
        general->tryLoadingSkinTranslation(n);
    ...
    ui->cvLineEdit->setText(getCvInfo(general->objectName()));
