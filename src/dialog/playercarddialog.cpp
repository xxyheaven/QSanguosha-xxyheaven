...
QWidget *PlayerCardDialog::createHandcardButton()
{
	if (player->isKongcheng()) {
		...
		if (Self == player || handcard_visible)
			visible_cards = player->getHandcards();
		else{
			QStringList visible_list = player->property("visible_handcards").toString().split("+");
			if (!visible_list.isEmpty()){
				foreach (QString id, visible_list) {
                    bool ok;
					int card_id = id.toInt(&ok);
		...
		int num = player->getHandcardNum() - visible_cards.length();
		if (num > 0) {
			PlayerCardButton *button = new PlayerCardButton(tr("Handcard"));
            button->setObjectName("handcard_button");
	    ...
	    for (int i = 0; i < visible_cards.length(); i += 2) {
            const Card *card = Sanguosha->getEngineCard(visible_cards.at(i)->getId());
            ...
            if (i < visible_cards.length() - 1) {
                card = Sanguosha->getEngineCard(visible_cards.at(i + 1)->getId());;
                button2 = new PlayerCardButton(card->getFullName());
				//button2->setToolTip("可见的手牌");
