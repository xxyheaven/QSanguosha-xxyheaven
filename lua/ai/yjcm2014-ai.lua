...
sgs.ai_skill_cardask["@sidi-remove"] = function(self, data)
		...
		and not (current:hasWeapon("crossbow") or current:hasSkill("paoxiao")) then
			for _, player in sgs.qlist(self.room:getOtherPlayers(current)) do
				...
				and self:slashIsEffective(slash, player) and (self:isWeak(player) or self.player:getPile("sidi"):length() > 1) then
					return "$" .. self.player:getPile("sidi"):first()
				end	
			end
		end	
	end
...
sgs.ai_skill_use["@@shenduan"] = function(self)
	local ids = self.player:getTag("shenduan_forAI"):toString():split("+")
...
sgs.ai_skill_cardask["@yonglve-use"] = function(self, data)
	local current = self.room:getCurrent()
	local card_id = self:askForCardChosen(current, "j", "dummyReason", sgs.Card_MethodDiscard)
	if self:isFriend(current) and card_id then
	    if not self:slashIsEffective(sgs.Sanguosha:cloneCard("slash"), current, self.player) then return "$" .. card_id end
		if not self:isWeak(current) or getKnownCard(current, self.player, "Jink") > 0 then return "$" .. card_id end
	elseif self:isEnemy(current) then
		...
		if self:isWeak(current) and current:getHp() == 1 and (sgs.card_lack[current:objectName()]["Jink"] == 1 or getCardsNum("Jink", current, self.player) == 0)
			...
			return "$" .. self:getCardRandomly(current, "j")
		end
	end
	return "."
end
...
sgs.ai_skill_invoke.bingyi = function(self, data)
	local color
	local invoke = true
	for _, c in sgs.qlist(self.player:getHandcards()) do
		if not color then
			color = c:isRed()
		elseif color ~= c:isRed() then
			invoke = false
			break
		end
	end
...
sgs.ai_skill_use["@@bingyiask!"] = function(self)
    self:sort(self.friends)
	local targets = {}
	for _, friend in ipairs(self.friends) do
	...
	return "@BingyiAskCard=.->" .. table.concat(targets, "+")
end

sgs.ai_card_intention.BingyiAskCard = function(self, card, from, tos)
