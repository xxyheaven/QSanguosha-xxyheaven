...
sgs.ai_skill_cardask["@qingjian-put"] = function(self, data)
    local select_cards = {}
    local rende_list = self.player:property("qingjian"):toString():split("+")
    for _, id in ipairs(rende_list) do
        ...
        if hcard then
            ...
            if c and friend and friend:objectName() ~= self.player:objectName() then
                table.insert(select_cards, c:getEffectiveId())
            end
        end
    end
...
sgs.ai_skill_use["@@qingjian_allot!"] = function(self, prompt)
    local rende_list = self.player:property("qingjian_give"):toString():split("+")
    for _, id in ipairs(rende_list) do
        local num_id = tonumber(id)
        local hcard = sgs.Sanguosha:getCard(num_id)
        if hcard then
            ...
            if c and friend then
                return "@QingjianAllotCard=" .. c:getEffectiveId() .. "->" .. friend:objectName()
            end
        end
    end
end
...
local function RendeArrange(self, cards, friends, enemies, unknowns, arrange)
    if #enemies > 0 then
...
sgs.ai_skill_use_func["RendeCard"] = function(card, use, self)
    local names = self.player:property("rende"):toString():split("+")
    local others = self.room:getOtherPlayers(self.player)
    ...
    while true do
        ...
        local target, to_give, group = RendeArrange(self, handcards, friends, enemies, unknowns, arrange)
    ...
    if max_target and type(arrange[max_name]) == "table" and #arrange[max_name] > 0 then
        ...
        for _,c in ipairs(arrange[max_name]) do
            if c:isKindOf("Slash") and self.player:getMark("drank") > 0 then
                local should_keep = true
                for _,keep in ipairs(to_keep) do
                    if keep:isKindOf("Slash") then
                        should_keep = false
                    end
                end
                if should_keep then table.insert(to_keep, c) end
            elseif c:isKindOf("Jink") and self:isWeak() then
                local should_keep = true
                for _,keep in ipairs(to_keep) do
                    if keep:isKindOf("Jink") then
                        should_keep = false
                    end
                end
                if should_keep then table.insert(to_keep, c) end
        ...
        local card_str = "@RendeCard="..table.concat(to_use, "+")
...
sgs.ai_skill_use["@@rende_basic"] = function(self)
    local choices = "slash+fire_slash+thunder_slash+jink+peach+analeptic"
    local choice_table = choices:split("+")
    local cards_value = {}
    
    if self.player:isWounded() and table.contains(choice_table, "peach") then
        local peach = sgs.Sanguosha:cloneCard("peach")
        if peach ~= nil then
            peach:deleteLater()
            local value = sgs.ai_use_value["Peach"] or 0
            value = value + self.player:getLostHp() - 3
            local val = { name = "peach", use_value = value }
            table.insert(cards_value, val)
        end
    end
    
    
    local canJijiang = function(self)
        if not self.player:hasLordSkill("jijiang") then
            return false
        end
        
        for _,fren in ipairs(self.friends_noself) do
            if fren:getKingdom() == "shu" then
                for _,c in sgs.qlist(fren:getHandcards()) do
                    if isCard("Slash", c, fren) or isCard("FireSlash", c, fren) or isCard("ThunderSlash", c, fren) then
                        return true
                    end
                end
            end
        ...
        if self:getCardsNum("slash") > 0 then return true end
    ...
    local slashs = {"slash=" .. "Slash"}
    if not (Set(sgs.Sanguosha:getBanPackages()))["maneuvering"] then
        table.insert(slashs, 2, "thunder_slash=" .. "ThunderSlash")
        table.insert(slashs, 2, "fire_slash=" .. "FireSlash")
    end
    for _,slashKind in ipairs(slashs) do
        if sgs.Slash_IsAvailable(self.player) then
            local slash = sgs.Sanguosha:cloneCard(slashKind:split("=")[1])
            slash:deleteLater()
            local dummy_use = {
                isDummy = true,
                to = sgs.SPlayerList()
            ...
            if dummy_use.card and dummy_use.to:length() > 0 then
                local value = sgs.ai_use_value[slashKind:split("=")[2]] or 0
                local val = { name = slashKind:split("=")[1], use_value = value }
                table.insert(cards_value, val)
            end
        end
    end
    
    local choice, choice_value
    for _,val in ipairs(cards_value) do
        if choice == nil or val.use_value > choice_value then
            choice = val.name
            choice_value = val.use_value
        end
    end
    
    if choice then
        local card, to = nil, {}
        if choice == "peach" or choice == "analeptic" then
            ...
            if dummy_use.card and dummy_use.to:length() > 0 then
                for _,p in sgs.qlist(dummy_use.to) do
                    table.insert(to, p:objectName())
                end
            end
        end
        return card:toString() .. "->" .. table.concat(to, "+")
    end
    return "."
end
...
sgs.ai_skill_cardask["@jijiang-slash"] = function(self, data, pattern, target)
    assert(target)
    sgs.jijiangsource = target
...
sgs.ai_skill_invoke.yijue_recover = function(self, data)
...
sgs.ai_skill_invoke.yajiao_discard = function(self, data)
    local card = sgs.Sanguosha:getCard(self.player:getMark("yajiao"))
    ...
    if current:isAlive() then
        ...
        if currentMayObtain then
            ...
            return (self:isFriend(current) == valuable) and false or true
        end
    end
    ...
    if nextAlive:hasSkill("luoshen") then
        ...
        return (self:isFriend(nextAlive) == valid) and true or false
    end
    if nextAlive:hasSkill("yinghun") and nextAlive:isWounded() then
        return self:isFriend(nextAlive) and false or true
    end
    ...
    if self:isFriend(nextAlive) and not self:willSkipDrawPhase(nextAlive) and not self:willSkipPlayPhase(nextAlive)
        ...
        for _, skill in ipairs(sgs.getPlayerSkillList(nextAlive)) do
            if sgs.ai_cardneed[skill:objectName()] and sgs.ai_cardneed[skill:objectName()](nextAlive, card) then return false end
        end
    end

    local trash = card:isKindOf("Disaster") or card:isKindOf("AmazingGrace") or card:isKindOf("GodSalvation")
    if trash and self:isEnemy(nextAlive) then return false end
    return true
end
...
sgs.ai_skill_use["@@lianying"] = function(self, prompt)
    ...
    if move.reason.m_skillName == "qizhi" and move.reason.m_playerId then
        local wangji = findPlayerByObjectName(self.room, move.reason.m_playerId)
        if wangji and self:isFriend(wangji) and wangji:getPhase() == sgs.Player_Play then
            return "@LianyingCard=.->" .. wangji:objectName()
        end
    end
...
function SmartAI:getWoundedFriend(maleOnly, include_self)
    ...
    local getCmpHp = function(p)
        ...
        if p:hasSkill("buqu") and p:getPile("buqu_chuang"):length() > 0 then hp = hp + math.max(0, 5 - p:getPile("buqu_chuang"):length()) end
...
chuli_skill.getTurnUseCard = function(self, inclusive)
    ...
    if self:needToThrowArmor() then return sgs.Card_Parse("@ChuliCard=.") end
    for _, card in ipairs(cards) do
        if not self:isValuableCard(card) then
            if card:isBlack() then return sgs.Card_Parse("@ChuliCard=.") end
        end
    end
    for _, card in ipairs(cards) do
        if not self:isValuableCard(card) and not (self.player:hasSkill("jijiu") and card:isRed() and self:getOverflow() < 2) then
            return sgs.Card_Parse("@ChuliCard=.")
        end
    end
end
...
local function target_for_liyu(self, lvbu)
    local enemies = {}
    local targets = self.room:getOtherPlayers(lvbu)
    local duel = sgs.Sanguosha:cloneCard("duel")
    targets:removeOne(self.player)
    for _, p in sgs.qlist(targets) do
        if not lvbu:isProhibited(p, duel) and self:isEnemy(p) then
            table.insert(enemies, p)
        end
    end
...
sgs.ai_skill_invoke.liyu_invoke = function(self, data)
    local prompt = data:toString()
    self.liyu_target = nil
    local lvbu = findPlayerByObjectName(self.room, prompt:split(":")[2])
    local target = target_for_liyu(self, lvbu)
    if target then
        self.liyu_target = target
        return true
    end
    return false
end
...
sgs.ai_skill_playerchosen.liyu = function(self, targets)
    local target = self.liyu_target
    if target and targets:contains(target) then
        return target
    else
        local enemies = {}
        for _, p in sgs.qlist(targets) do
...
sgs.ai_skill_invoke.qiaomeng = function(self, data)
    local target = data:toPlayer()
    if self:isFriend(target) then return target:getArmor() and self:needToThrowArmor(target) end
    return not self:doNotDiscard(target, "e")
end
...
jianyan_skill.getTurnUseCard = function(self)
    if self.player:hasUsed("JianyanCard") then return end
    
    local parseJiantanCard = function(choice)
        return sgs.Card_Parse("@JianyanCard=.:" .. choice)
    end

    for _, friend in ipairs(self.friends) do
        if friend:isMale() and not hasManjuanEffect(friend) and not self:needKongcheng(friend, true) then
            if friend:getHandcardNum() < 3 and friend:getEquips():length() < 2 then return parseJiantanCard("equip") end
        end
    end
    for _, friend in ipairs(self.friends) do
        if friend:isMale() and not hasManjuanEffect(friend) and not self:needKongcheng(friend, true) then
            if friend:hasSkills("jizhi|nosjizhi|jilve") then return parseJiantanCard("trick") end
        end
    end
    local rand = math.random(0, 100)
    if rand > 60 then return parseJiantanCard("trick")
    elseif rand > 35 then return parseJiantanCard("red")
    elseif rand > 10 then return parseJiantanCard("equip")
    elseif rand > 2 then return parseJiantanCard("basic")
    else return parseJiantanCard("black")
    end
end
...
sgs.ai_playerchosen_intention.jianyan = function(self, from, to)
    if not self:needKongcheng(to, true) and not hasManjuanEffect(to) then sgs.updateIntention(from, to, -50) end
end
