...
sgs.ai_skill_cardask["@qianxi-discard"] = function(self, data)
	local to_discard = self:askForDiscard("qianxi", 1, 1, false, true)
...
sgs.ai_skill_use_func.GongqiCard = function(card, use, self)
	...
	if subcard:isKindOf("SilverLion") and self.room:getCardPlace(id) == sgs.Player_PlaceHand and self.player:isWounded() then
		use.card = subcard
		return
	end
	use.card = card
...
nosanxu_skill = {}
nosanxu_skill.name = "nosanxu"
table.insert(sgs.ai_skills, nosanxu_skill)
nosanxu_skill.getTurnUseCard = function(self)
	if self.player:hasUsed("NosAnxuCard") then return nil end
	card = sgs.Card_Parse("@NosAnxuCard=.")
	return card

end

sgs.ai_skill_use_func.NosAnxuCard = function(card,use,self)
	if #self.enemies == 0 then return end
	local intention = 50
	local friends = {}
	for _, friend in ipairs(self.friends_noself) do
...
sgs.ai_card_intention.NosAnxuCard = 0
sgs.ai_use_priority.NosAnxuCard = 9.6
...
sgs.ai_skill_playerchosen.lihuo = sgs.ai_skill_playerchosen.slash_extra_targets
...
sgs.ai_card_intention.ChunlaoCard = sgs.ai_card_intention.Peach
...
--room->askForExchange(to, "anxu", 1, 1, false, "@anxu-give:" + source->objectName() + ":" + from->objectName())
sgs.ai_skill_discard["anxu"] = function(self, discard_num, min_num, optional, include_equip)
    local others = self.room:getOtherPlayers(self.player)
    local target = nil
    for _,p in sgs.qlist(others) do
        if p:hasFlag("anxu_target") then
            target = p
            break
        end
    end
...
--room->askForChoice(source, "anxu", choices)
sgs.ai_skill_choice["anxu"] = function(self, choices, data)
