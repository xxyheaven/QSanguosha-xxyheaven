...
function SmartAI:slashIsEffective(slash, to, from, ignore_armor)
	...
	if to:getMark("#zhichi") > 0 then return false end
			...
			if not self:isFriend(to) or self:needToThrowArmor(to) then
				if not (self:isEnemy(to) and self:doNotDiscard(to)) then
                    ...
                    if from:hasSkill("pojun") then
                        ...
                        if table.contains(ids, armor_id) then ignore_armor = true end
                    end
					if from:hasSkill("jianchu") then
                        local ids = self:askForCardChosen(to, "he", "jianchu")
                        if id == to:getArmor():getEffectiveId() then ignore_armor = true end
                    end
				end
			end
...
function sgs.isJinkAvailable(from, to, slash, judge_considered)
	return (not judge_considered and from:hasSkills("tieji|nostieji"))
			or (from:hasSkill("liegong") and to:getHandcardNum() <= from:getHandcardNum())
		...
		if (not use.current_targets or not table.contains(use.current_targets, target:objectName()))
			...
			if not use.isDummy then
				...
				if self.player:hasSkill("jilve") and self.player:getMark("@bear") > 0 and not self.player:hasFlag("JilveWansha") and target:getHp() == 1 and not self.room:getCurrent():hasSkill("wansha")
					and (target:isKongcheng() or getCardsNum("Jink", target, self.player) < 1 or sgs.card_lack[target:objectName()]["Jink"] == 1) and #self.enemies > 1 then
					use.card = sgs.Card_Parse("@JilveCard=.:wansha")
					if use.to then use.to = sgs.SPlayerList() end
					return
				end
...
sgs.ai_card_intention.Slash = function(self, card, from, tos)
	...
	if card:hasFlag("nosjiefan-slash") or card:hasFlag("Global_NoAIIntention") then return end
	...
	for _, to in ipairs(tos) do
		...
		if to:hasSkills("nosleiji|leiji") and (getCardsNum("Jink", to, from) > 0 or to:hasArmorEffect("eight_diagram")) and not self:hasHeavySlashDamage(from, card, to)
			and (hasExplicitRebel(self.room) or sgs.explicit_renegade) and not self:canLiegong(to, from, card) then value = 0 end
...
sgs.ai_skill_cardask["slash-jink"] = function(self, data, pattern, target)
	...
	if self:isFriend(target) then
		...
		if not target:hasSkill("jueqing") then
			if (target:hasSkill("nosrende") or target:hasSkill("rende")) and self.player:hasSkill("jieming") then return "." end
...
function SmartAI:useCardPeach(card, use)
	...
	if (self.player:hasSkill("nosrende") or self.player:hasSkill("rende")) and self:findFriendsByType(sgs.Friend_Draw) then return end
...
function SmartAI:getDangerousCard(who)
	...
	if weapon and (weapon:isKindOf("Crossbow") or weapon:isKindOf("GudingBlade")) then
		for _, friend in ipairs(self.friends) do
			if weapon:isKindOf("Crossbow") and who:distanceTo(friend) <= 1 and getCardsNum("Slash", who, self.player) > 1 then
	...
	if weapon then
		for _, friend in ipairs(self.friends) do
			if who:distanceTo(friend) <= who:getAttackRange() and who:distanceTo(friend) > who:getAttackRange(false) and self:isWeak(friend) and not self:doNotDiscard(who, "e", true) then
				return weapon:getEffectiveId()
			end
		end
	end
...
function SmartAI:getValuableCard(who)
	...
	for _, equip in ipairs(equips) do
		...
		if who:hasSkills("qixi|duanliang|guidao]huomo") and equip:isBlack() then return equip:getEffectiveId() end
	...
	if offhorse then
		if who:hasSkills("kuanggu|duanbing") then
			return offhorse:getEffectiveId()
		end
	end
			...
			for _, slash in ipairs(slashs) do
				if self:slashIsEffective(slash, enemy) and self.player:canSlash(enemy, slash) and self:slashIsAvailable() then
					...
					if getCardsNum("Jink", enemy) < 1
						...
						or self:canLiegong(enemy, self.player, slash)
...
sgs.ai_skill_playerchosen.wooden_ox_move = function(self, targets)
	return self.wooden_ox_assist
end

sgs.ai_playerchosen_intention.wooden_ox_move = -10
