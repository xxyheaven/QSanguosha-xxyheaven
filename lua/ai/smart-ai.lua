...
function setInitialTables()
    ...
    sgs.ignore_armor_skill = "moukui|pojun|jianchu"
    ...
    sgs.wizard_skill =      "guicai|guidao|jilve|tiandu|luoying|noszhenlie|huanshi|midao"
    sgs.wizard_harm_skill =     "guicai|guidao|jilve|midao"
    sgs.priority_skill =        "dimeng|haoshi|qingnang|nosjizhi|jizhi|guzheng|qixi|jieyin|guose|duanliang|jujian|fanjian|neofanjian|lijian|" ..
                        ...
                        "qice|yinling|qingcheng|houyuan|zhaoxin|shuangren|zhaxiang|xiansi|junxing|bifa|yanyu|qiangwu|jgtianyun|dingpan"
    ...
    sgs.recover_skill =     "nosrende|rende|kofkuanggu|kuanggu|zaiqi|jieyin|qingnang|yinghun|hunzi|shenzhi|longhun|nosmiji|zishou|ganlu|shangshi|" ..
    ...
    for _, aplayer in sgs.qlist(global_room:getAllPlayers()) do
        ...
        if aplayer:isLord() or (aplayer:hasShownRole() and aplayer:getRole() == "loyalist") then
...
function sgs.getDefense(player)
    ...
    if player:hasSkill("yishe+bushi") then defense = defense + 2 end
...
function SmartAI:getUseValue(card)
    ...
    elseif card:getTypeId() == sgs.Card_TypeBasic then
        if card:isKindOf("Slash") then
			...
			if card:getSkillName() == "fanghun" then v = v + 1 end
...
function SmartAI:getDynamicUsePriority(card)
    ...
    if self.player:hasSkill("jianying") and (self.player:getMark("JianyingSuit") == card:getSuit() + 1 or self.player:getMark("JianyingNumber") == card:getNumber()) then
    ...
    if self.player:hasSkill("zhidao") and not self.player:hasSkill("jueqing")
...
function SmartAI:sortByUsePriority(cards, inverse)
    local compare_func = function(a, b)
...
function sgs.evaluatePlayerRole(player)
    ...
    if sgs.isRolePredictable() or player:hasShownRole() then return player:getRole() end
...
sgs.ai_card_intention.general = function(from, to, level)
    ...
    if from:isLord() or level == 0 or from:hasShownRole() then return end
    ...
    if sgs.debugai and global_room:getTag("humanCount") and global_room:getTag("humanCount"):toInt() ==1 then
        local diffarr = {
...
function SmartAI:mayBeLord(player)
    ...
    return player:isLord() or (sgs.evaluatePlayerRole(player) == "loyalist")
end
...
function SmartAI:objectiveLevel(player)
    ...
    if self.role == "renegade" then
        ...
        elseif process:match("dilemma") then
            ...
            elseif target_role == "loyalist" or target_role == "renegade" then return 1
    ...
    if self.player:getRole() == "lord" or self.role == "loyalist" then
        ...
        if rebel_num == 0 then
            ...
            if self.player:getRole() == "lord" and not self.player:hasFlag("stack_overflow_jijiang") and player:getHp() <= 2 and self:hasHeavySlashDamage(self.player, nil, player) then
                return 0
            end
...
function SmartAI:updatePlayers(clear_flags, update)
    ...
    if (sgs.isRolePredictable() or self.player:hasShownRole()) and sgs.ai_role[self.player:objectName()] ~= self.player:getRole()
...
sgs.ai_choicemade_filter.Nullification.general = function(self, player, promptlist)
    ...
    local positive = promptlist[4]
    local source = findPlayerByObjectName(global_room, target_objectName)
    local intention = getTrickIntention(trick_class, source)
    if positive == "true" then
        intention = - intention
    end
    if player:objectName() ~= target_objectName then
        sgs.updateIntention(player, source, intention)
    end
end
...
function SmartAI:askForChoice(skill_name, choices, data)
    ...
    elseif type(choice) == "function" then
        ...
        --else
			...
			if (skill_name == "GameRule_TriggerOrder") then
				for index, achoice in ipairs(choice_table) do
					if achoice == "cancel" then table.remove(choice_table, index) break end
				end
			end
...
function SmartAI:askForNullification(trick, from, to, positive)
    ...
    if self.player:hasSkill("wumou") then
        if self.player:getMark("@wrath") == 0 and (self:isWeak() or self.player:getRole() == "lord") then return nil end
            ...
            if self:isFriend(NP) then
                ...
                if peach_num == 0 and not self:willSkipPlayPhase(NP) then
                    if exnihilo_num > 0 then
                        if NP:hasSkills("nosjizhi|jizhi|nosrende|rende|zhiheng")
...
function SmartAI:askForCardsChosen(who, flags, reason, min_num, max_num, method)
    local isDiscard = (method == sgs.Card_MethodDiscard)
    ...
    result = {}
    if reason == "zhidao" then
        if not who:isKongcheng() then
            local id = self:askForCardChosen(who, "h", reason, method)
            if id > -1 then
                table.insert(result, id)
            end
        end
        if who:hasEquip() then
            local id = self:askForCardChosen(who, "e", reason, method)
            if id > -1 then
                table.insert(result, id)
            end
        end
        if not who:getJudgingArea():isEmpty() then
            local id = self:askForCardChosen(who, "j", reason, method)
            if id > -1 then
                table.insert(result, id)
            end
        end
...
function sgs.ai_skill_cardask.nullfilter(self, data, pattern, target)
    ...
    if target and target:hasSkill("guagu") and self.player:getRole() == "lord" then return "." end
...
function HandcardNumCorrect(player)
    local n = player:getHandcardNum()
    local rende_list = player:property("rende"):toString():split("+")
    if #rende_list > 0 then
        n = #rende_list
    end
    local rende_cards = player:property("rende_give"):toString():split("+")
    n = n + #rende_cards
    return n
end
...
function SmartAI:getCardNeedPlayer(cards, include_self)
    ...
    for _, player in ipairs(friends_table) do
        ...
        if player:hasSkills("keji|qiaobian|shensu") or player:getHp() - HandcardNumCorrect(player) >= 3
    ...
    if specialnum > 1 and #cardtogivespecial == 0 and self.player:hasSkill("nosrende") and self.player:getPhase() == sgs.Player_Play then
        ...
        if HandcardNumCorrect(self.player) > #cardtogivespecial and redcardnum > 0 then
            for _, hcard in ipairs(cardtogivespecial) do
    ...
    for _, friend in ipairs(friends) do
        if self:isWeak(friend) and HandcardNumCorrect(friend) < 3  then
    ...
    if (self.player:hasSkill("nosrende") and self.player:isWounded() and self.player:getMark("nosrende") < 2) then
        if (HandcardNumCorrect(self.player) < 2 and self.player:getMark("nosrende") == 0) then return end
    end
    if (self.player:hasSkill("rende") and self.player:isWounded() and self.player:getMark("rende") < 2) then
        if (HandcardNumCorrect(self.player) < 2 and self.player:getMark("rende") == 0) then return end

        if ((HandcardNumCorrect(self.player) == 2 and self.player:getMark("rende") == 0) or
            (HandcardNumCorrect(self.player) == 1 and self.player:getMark("rende") == 1)) and self:getOverflow() <= 0 then
    ...
    for _, friend in ipairs(friends) do
        if friend:hasSkills("jijiu|jieyin") and HandcardNumCorrect(friend) < 4 then
            for _, hcard in ipairs(cards) do
    ...
    --Crossbow
    for _, friend in ipairs(friends) do
        if friend:hasSkills("longdan|wusheng|keji|chixin") and not self:hasCrossbowEffect(friend) and HandcardNumCorrect(friend) >= 2 then
            for _, hcard in ipairs(cards) do
    ...
    if self.role == "lord" and self.player:hasLordSkill("jijiang") then
        for _, friend in ipairs(friends) do
            if friend:getKingdom() == "shu" and HandcardNumCorrect(friend) < 3  then
                for _, hcard in ipairs(cardtogive) do
                    if isCard("Slash", hcard, friend) then
                        return hcard, friend
                    end
    ...
    for _, hcard in ipairs(cardtogive) do
        for _, friend in ipairs(friends) do
            if not self:needKongcheng(friend, true) and not friend:hasSkill("manjuan") and not self:willSkipPlayPhase(friend) and self:hasSkills(sgs.priority_skill,friend)
                and (self:getOverflow() > 0 or HandcardNumCorrect(self.player) > 3) and HandcardNumCorrect(friend) <= 3 then
                return hcard, friend
            end
        end
    end

    local shoulduse = self.player:isWounded() and (self.player:hasSkill("rende") and self.player:getMark("rende") < 2)
    ...
    for _, hcard in ipairs(cardtogive) do
        for _, friend in ipairs(friends) do
            if not self:needKongcheng(friend, true) and not friend:hasSkill("manjuan") then
                if HandcardNumCorrect(friend) <= 3 and (self:getOverflow() > 0 or HandcardNumCorrect(self.player) > 3 or shoulduse) then
                    return hcard, friend
                end
            end
        end
    ...
    for _, hcard in ipairs(cardtogive) do
        for _, friend in ipairs(friends) do
            if (not self:needKongcheng(friend, true) or #friends == 1) and not friend:hasSkill("manjuan") then
                if self:getOverflow() > 0 or HandcardNumCorrect(self.player) > 3 or shoulduse then
                    return hcard, friend
                end
            end
        end
...
function SmartAI:askForPindian(requestor, reason)
    local passive = { "mizhao", "lieren" , "jianshu"}
...
function SmartAI:getOverflow(player, getMaxCards)
    ...
    local MaxCards = 0
    if player:hasSkill("qiaobian") then MaxCards = math.max(HandcardNumCorrect(self.player) - 1, player:getMaxCards()) end
    if player:hasSkill("keji") and not player:hasFlag("KejiSlashInPlayPhase") then MaxCards = HandcardNumCorrect(self.player) end
    if player:hasSkill("zaoyao") then MaxCards = HandcardNumCorrect(self.player) end
    ...
    return HandcardNumCorrect(self.player) - MaxCards
end
...
function SmartAI:canRetrial(player, to_retrial, reason)
    ...
    if player:hasSkill("guicai") and not player:isNude() then return true end
    ...
    if player:hasSkill("midao") and player:getPile("rice"):length() > 0 then return true end
    return
end
...
function SmartAI:needRetrial(judge)
    ...
    if reason == "huituo" then
        if self:isFriend(who) then
            return judge.card:isBlack() and who:isWounded()
        elseif self:isEnemy(who) then
            return judge.card:isRed() and who:isWounded()
        end
    end
...
function SmartAI:getCardJudgeValue(reason, card, target)
    local callback = sgs.ai_judge_value[reason]
    if type(callback) == "function" then
        local value = callback(self, card, target)
        return value
    end
    return 0
end
...
function SmartAI:getRetrialCardId(cards, judge, self_card)
    ...
    for _, card in ipairs(cards) do
        ...
        elseif self:isFriend(who) and judge:isGood(card_x)
            ...
            local value = self:getCardJudgeValue(reason, card, who)
    ...
    if best_card ~= nil then return best_card:getEffectiveId() end
...
--for jvshou
    ...
    if to:hasSkill("shibei") then
        if to:hasFlag("ShibeiFirst") then
			damage = damage + 1
        elseif damage < to:getHp() then
            damage = damage - 1
            if damage == 0 and not DoubleDamage then return false end
        end
    end
--for zhanglu
    if to:hasSkills("yishe+bushi") then
        if to:getPile("rice"):length() == 1 and damage < to:getHp() then
            damage = damage - 1
...
function SmartAI:getCardId(class_name, player, acard)
    ...
    if #viewArr > 0 or #cardArr > 0 then
		...
		if player:hasSkill("chongzhen") and viewCard and viewCard:getSkillName() == "longdan" then
			return (viewas or cardid)
		elseif player:hasSkill("fanghun") and viewCard and viewCard:getSkillName() == "fanghun" then
			return (viewas or cardid)
		else 
...
function getCardsNum(class_name, player, from)
    ...
    elseif class_name == "Peach" then
        ...
        elseif player:hasSkill("chunlao") and not player:getPile("wine"):isEmpty() then
            return num + 1
...
function SmartAI:aoeIsEffective(card, to, source)
    ...
    if (source:hasSkill("wuyan") or to:hasSkill("wuyan")) and not source:hasSkill("jueqing") then
        return false
    end
...
function SmartAI:getDistanceLimit(card, from)
    ...
    if (card:isKindOf("Snatch") or card:isKindOf("SupplyShortage")) and not card:hasFlag("Global_NoDistanceChecking") then
...
function getLord(player)
    ...
    elseif mode == "08_zdyj" then
        if room:getLord() then return room:getLord() end
        for _, p in sgs.qlist(room:getAllPlayers()) do
            if p:getRole() == "loyalist" and p:hasShownRole() then return p end
        end
    end
...
function SmartAI:hasTrickEffective(card, to, from)
    ...
    if to:getMark("#zhichi") > 0 and not card:isKindOf("DelayedTrick") then return false end
...
function SmartAI:useTrickCard(card, use)
    ...
    if card:isKindOf("AOE") then
        ...
        if self:hasSkill("luanji") and self.player:getRole() == "lord" and sgs.turncount < 2 then
        ...
        if mode:find("p") and mode >= "04p" then
            if self.player:getRole() == "lord" and sgs.turncount < 2 and card:isKindOf("ArcheryAttack") and self:getOverflow() < 1
...
function SmartAI:useEquipCard(card, use)
    ...
    if same then
        if (self:hasSkill("nosgongqi") and self:slashIsAvailable())
            ...
            or (self.player:hasSkill("rende") and self:findFriendsByType(sgs.Friend_Draw))
    ...
    if card:isKindOf("Weapon") then
        ...
        if self.player:hasSkill("nosrende") or self.player:hasSkill("rende") then
            for _, friend in ipairs(self.friends_noself) do
    ...
    elseif card:isKindOf("Armor") then
        ...
        if (self.player:hasSkill("nosrende") or self.player:hasSkill("rende")) and self:evaluateArmor(card) < 4 then
            for _, friend in ipairs(self.friends_noself) do
...
function SmartAI:needRende()
    return self.player:getLostHp() > 1 and self:findFriendsByType(sgs.Friend_Draw)
            and ((self.player:hasSkill("nosrende") and self.player:getMark("nosrende") < 2)
                or (self.player:hasSkill("rende") and self.player:getMark("rende") < 2))
end


function getBestHp(player)
    local arr = {ganlu = 1, yinghun = 2, miji = 1, quji = 1, baobian = math.max(0, player:getMaxHp() - 3)}
    if player:hasSkill("longhun") and player:getCards("he"):length() > 2 then return 1 end
    if player:hasSkill("hunzi") and player:getMark("hunzi") == 0 and not player:hasSkill("chanyuan") then return 1 end
    for skill,dec in pairs(arr) do
        if player:hasSkill(skill) then
            return math.max((player:isLord() and 3 or 2), player:getMaxHp() - dec)
        end
    end
	if player:hasSkills("qianxin") and player:getMark("qianxin") == 0 then return (player:getMaxHp() - 1) end
...
function SmartAI:needToLoseHp(to, from, isSlash, passive, recover)
    ...
    if not passive then
        if to:getMaxHp() > 2 then
            ...
            if to:hasSkills("nosrende") and not self:willSkipPlayPhase(to) and self:findFriendsByType(sgs.Friend_Draw, to) then n = math.min(n, to:getMaxHp() - 1) end
            ...
            if to:hasSkill("canshi") and self.room:alivePlayerCount() >= 3 then n = math.min(n, to:getMaxHp() - 1) end
			local sp_caoren = self.room:findPlayerBySkillName("lizhan")
			if sp_caoren and self:isFriend(sp_caoren, to) then n = math.min(n, to:getMaxHp() - 1) end
        end
    end
...
function SmartAI:findPlayerToDiscard(flags, include_self, isDiscard, players, return_table)
    ...
    if flags:match("e") then
        ...
        for _, enemy in ipairs(enemies) do
            if enemy:hasSkills("jijiu|beige|mingce|weimu") and not self:doNotDiscard(enemy, "e") then
    ...
    if flags:match("h") then
        for _, enemy in ipairs(enemies) do
            ...
            if #cards <= 2 and not enemy:isKongcheng() and not (enemy:hasSkills("tuntian+zaoxian") and enemy:getPhase() == sgs.Player_NotActive) then
                for _, cc in ipairs(cards) do
                    if (cc:hasFlag("visible") or cc:hasFlag(flag)) and (cc:isKindOf("Peach") or (cc:isKindOf("Analeptic") and self:isWeak(enemy))) and (not isDiscard or self.player:canDiscard(enemy, cc:getId())) then
                        table.insert(player_table, enemy)
                    end
                end
            end
