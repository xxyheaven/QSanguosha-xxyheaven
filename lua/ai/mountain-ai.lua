...
--巧变（判定阶段）
sgs.ai_skill_cardask["#qiaobian-1"] = function(self, data)
	local to_discard = {}
	...
	if not self.player:isSkipped(sgs.Player_Judge) then
		if not self.player:containsTrick("YanxiaoCard") then
			...
			elseif self.player:containsTrick("supply_shortage") then
				if self.player:getHp() > self.player:getHandcardNum() then return "$" .. to_discard[1] end
			...
			elseif self.player:containsTrick("indulgence") then
				if self.player:getHandcardNum() > 3 or self.player:getHandcardNum() > self.player:getHp() - 1 then return "$" .. to_discard[1] end
...
--巧变（摸牌阶段）
sgs.ai_skill_cardask["#qiaobian-2"] = function(self, data)
	local to_discard = {}
	local cards = self.player:getHandcards()
	cards = sgs.QList2Table(cards)
	self:sortByUseValue(cards, true)
	...
	if card == nil then return "." end
	table.insert(to_discard, card:getEffectiveId())
	if not self.player:isSkipped(sgs.Player_Draw) and not self.player:hasSkills("tuxi|nostuxi") then
		local cardstr = sgs.ai_skill_use["@@nostuxi"](self, "@nostuxi")
		if cardstr:match("->") then
			local targetstr = cardstr:split("->")[2]
			local targets = targetstr:split("+")
...
--巧变（出牌阶段）
sgs.ai_skill_cardask["#qiaobian-3"] = function(self, data)
	local to_discard = {}
	local cards = self.player:getHandcards()
	cards = sgs.QList2Table(cards)
	self:sortByUseValue(cards, true)
...
--巧变（弃牌阶段）
sgs.ai_skill_cardask["#qiaobian-4"] = function(self, data)
	local to_discard = {}
	local cards = self.player:getHandcards()
	cards = sgs.QList2Table(cards)
	self:sortByUseValue(cards, true)
...
sgs.ai_skill_askforag.qiaobian = function(self, card_ids)
	for _,p in sgs.qlist(self.room:getAlivePlayers()) do
		if p:hasFlag("QiaobianTarget") then
			return card_for_qiaobian(self, p, "card"):getEffectiveId()
		end
	end
	return -1
end

sgs.ai_skill_use["@@qiaobianask"] = function(self, prompt)
	self:updatePlayers()
	local cards = self.player:getHandcards()
	cards = sgs.QList2Table(cards)
	self:sortByUseValue(cards, true)
	...
	if prompt == "@qiaobian-2" then
		...
		if cardstr:match("->") then
			...
			return "@QiaobianAskCard=.->" .. targetstr
		else
			return "."
		end
	end
	...
	if prompt == "@qiaobian-3" then
		...
		for _, friend in ipairs(self.friends) do
			if not friend:getCards("j"):isEmpty() and not friend:containsTrick("YanxiaoCard") and card_for_qiaobian(self, friend, ".") then
				...
				return ("@QiaobianAskCard=.->%s+%s"):format(friend:objectName(), card_for_qiaobian(self, friend, "target"):objectName())
			end
		end
		...
		for _, enemy in ipairs(self.enemies) do
			if not enemy:getCards("j"):isEmpty() and enemy:containsTrick("YanxiaoCard") and card_for_qiaobian(self, enemy, ".") then
				-- return "@QiaobianCard=" .. card:getEffectiveId() .."->".. friend:objectName()
				return ("@QiaobianAskCard=.->%s+%s"):format(enemy:objectName(), card_for_qiaobian(self, enemy, "target"):objectName())
			end
		end
		...
		if #targets > 0 then
			...
			return ("@QiaobianAskCard=.->%s+%s"):format(targets[#targets]:objectName(), card_for_qiaobian(self, targets[#targets], "target"):objectName())
		end
	end
...
sgs.ai_card_intention.QiaobianAskCard = function(self, card, from, tos)
...
sgs.ai_skill_invoke.fangquan = function(self, data)
	...
	if not to_discard then
		to_discard = cards[1]:getEffectiveId()
	end
...
sgs.ai_skill_use["@@fangquanask"] = function(self, prompt)
	local cards = sgs.QList2Table(self.player:getHandcards())
	self:sortByKeepValue(cards)
	if sgs.current_mode_players.rebel == 0 then
		local lord = self.room:getLord()
		if lord and self:isFriend(lord) and lord:objectName() ~= self.player:objectName() then
			return "@FangquanAskCard=" .. cards[1]:getEffectiveId() .. "->" .. lord:objectName()
		end
	end

	local AssistTarget = self:AssistTarget()
	if AssistTarget and not self:willSkipPlayPhase(AssistTarget) then
		return "@FangquanAskCard=" .. cards[1]:getEffectiveId() .. "->" .. AssistTarget:objectName()
	end
	...
	for _, target in ipairs(self.friends_noself) do
		if not target:hasSkill("dawu") and target:hasSkills("yongsi|zhiheng|" .. sgs.priority_skill .. "|shensu")
			and (not self:willSkipPlayPhase(target) or target:hasSkill("shensu")) then
			return "@FangquanAskCard=" .. cards[1]:getEffectiveId() .. "->" .. target:objectName()
		end
	end
...
sgs.ai_skill_invoke.guzheng = function(self, data)
	local n = self.player:getMark("guzheng_length")
	if self:isLihunTarget(self.player, n - 1) then return false end
	...
	local invoke = (self:isFriend(player) and not (player:hasSkill("kongcheng") and player:isKongcheng()))
					or (n >= 3 and not self.player:hasSkill("manjuan"))
					or (n == 2 and not self:hasSkills(sgs.cardneed_skill, player) and not self.player:hasSkill("manjuan"))
...
sgs.ai_skill_invoke.guzheng_obtain = true
