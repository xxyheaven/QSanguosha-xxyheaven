...
sgs.ai_skill_invoke.qinyin = function(self, data)
	...
	for _, friend in ipairs(self.friends) do
		...
		if self:isWeak(friend) then
			...
			if friend:getHp() <= 1 and not friend:hasSkill("buqu") or friend:getPile("buqu_chuang"):length() > 4 then
...
sgs.ai_skill_invoke.jilve_jizhi = function(self, data)
	...
	return use or (card and card:isKindOf("ExNihilo"))
end
...
jilve_skill.getTurnUseCard = function(self)
	if self.player:getMark("@bear") < 1 or (self.player:hasFlag("JilveWansha") and self.player:hasUsed("ZhihengCard")) then return end
	...
	if not wanshadone and not self.player:hasSkill("wansha") then
		...
		for _, enemy in ipairs(self.enemies) do
			if not (enemy:hasSkill("kongcheng") and enemy:isKongcheng()) and self:isWeak(enemy) and self:damageMinusHp(self, enemy, 1) > 0
				...
				return sgs.Card_Parse("@JilveCard=.:wansha")
			end
		end
	end
	if not self.player:hasUsed("ZhihengCard") then
		...
		local cards = sgs.QList2Table(self.player:getHandcards())
		if dummy_use.card then return sgs.Card_Parse("@JilveCard=" .. table.concat(sgs.QList2Table(dummy_use.card:getSubcards()), "+") .. ":zhiheng") end
